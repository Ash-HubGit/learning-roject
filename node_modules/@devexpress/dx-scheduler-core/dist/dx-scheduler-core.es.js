...
var addDateToKey = function (prevKey, momentDate) { return "".concat(prevKey, "_").concat(momentDate.toDate().toString()); };
...
var calculateAppointmentLeftAndWidth = function (appointmentNodes, blocks, appointmentNode, maxRight, indirectChildLeftOffset, defaultLeft) {
    var _a = appointmentNode, hasDirectChild = _a.hasDirectChild, treeDepth = _a.treeDepth, isDirectChild = _a.isDirectChild, parentIndex = _a.parent, children = _a.children, blockIndex = _a.blockIndex;
...
var calculateBlockSizeByEndDate = function (appointmentNodes, subTreeRoot, blockEndDate) {
    ...
    var maxSize = Math.max.apply(Math, __spreadArray([], __read(children.map(function (childIndex) { return calculateBlockSizeByEndDate(appointmentNodes, appointmentNodes[childIndex], blockEndDate); })), false));
...
var calculateSingleBlockTotalSize = function (blocks, block) {
    ...
    return Math.max.apply(Math, __spreadArray([], __read(children.map(function (childIndex) { return calculateSingleBlockTotalSize(blocks, blocks[childIndex]); })), false)) + size;
};
...
var calculateSingleBlockLeftLimit = function (blocks, appointments, block) {
    ...
    return Math.min.apply(Math, __spreadArray([], __read(children.map(function (childIndex) { return calculateSingleBlockLeftLimit(blocks, appointments, blocks[childIndex]); })), false));
};
...
var calculateRectByDateAndGroupIntervals = function (type, intervals, rectByDates, rectByDatesMeta, viewMetaData) {
    ...
    var grouped = sorted.reduce((function (acc, sortedGroup) { return __spreadArray(__spreadArray([], __read(acc), false), __read(findOverlappedAppointments(sortedGroup, isHorizontal)), false); }), []);
...
excludedDays, cellDuration) { return [
    appointments
        ...
        .reduce(function (acc, appointment) { return (__spreadArray(__spreadArray([], __read(acc), false), __read(sliceAppointmentByDay(appointment, cellDuration)), false)); }, [])
...
var calculateMonthDateIntervals = function (appointments, leftBound, rightBound) { return [
    appointments
        ...
        .reduce(function (acc, appointment) { return (__spreadArray(__spreadArray([], __read(acc), false), __read(sliceAppointmentByWeek({ left: moment(leftBound), right: moment(rightBound) }, appointment, DAY_COUNT)), false)); }, []),
]; };
...
var calculateAllDayDateIntervals = function (appointments, leftBound, rightBound, excludedDays) { return [
    appointments
        ...
        .reduce(function (acc, appointment) { return (__spreadArray(__spreadArray([], __read(acc), false), __read(sliceAppointmentsByBoundaries(appointment, leftBound, rightBound, excludedDays)), false)); }, []),
]; };
...
var getCurrentGroup = function (groups, resources, index, group) {
    ...
    return groups.reduceRight(function (groupAcc, groupsRow, rowIndex) {
        ...
        return __spreadArray(__spreadArray([], __read(groupAcc), false), [
            currentInstance,
        ], false);
    }, [group]);
};
...
var rearrangeResourceIds = function (mainResource, appointment, mainResourceId) {
    ...
    return __spreadArray([
        mainResourceId
    ], __read(appointment.dataItem[mainResource.fieldName].filter(function (id) { return id !== mainResourceId; })), false);
};
    ...
    return resources
        ...
        if (!resource.allowMultiple) {
            return acc.reduce(function (accumulatedAppointments, currentAppointment) {
                var _a;
                return __spreadArray(__spreadArray([], __read(accumulatedAppointments), false), [
                    __assign(__assign({}, currentAppointment), (_a = {}, _a[resourceField] = currentAppointment.dataItem[resourceField], _a)),
                ], false);
            }, []);
        }
        return acc.reduce(function (accumulatedAppointments, currentAppointment) { return __spreadArray(__spreadArray([], __read(accumulatedAppointments), false), __read(currentAppointment.dataItem[resourceField].map(function (resourceValue) {
            var _a;
...
var generateMultipleResourceKey = function (previousKey, resourceValue) { return "".concat(previousKey, "_").concat(resourceValue); };
...
var getGroupsFromResources = function (sortedAndFilteredResources) { return sortedAndFilteredResources.reduce(function (acc, resource, index) {
    ...
    return __spreadArray(__spreadArray([], __read(acc), false), [
        acc[index - 1].reduce(function (currentResourceNames) { return __spreadArray(__spreadArray([], __read(currentResourceNames), false), __read(resource.instances.map(function (instance) { return getGroupFromResourceInstance(instance); })), false); }, []),
    ], false);
}, []); };
...
var expandHorizontallyGroupedCells = function (viewCellsData, groups, sortedResources) { return getGroupsLastRow(groups).reduce(function (acc, group, index) {
    ...
    return acc.map(function (item, id) { return __spreadArray(__spreadArray([], __read(item), false), __read(addGroupInfoToCells(group, groups, sortedResources, viewCellsData[id], index, false, HORIZONTAL_GROUP_ORIENTATION)), false); });
}, [[]]); };
...
var expandGroups = function (appointments, grouping, resources, groups, excludedDays, sliceByDay) {
    ...
    var slicedAppointments = sliceByDay ?
        appointments[0].reduce(function (acc, appointment) { return (__spreadArray(__spreadArray([], __read(acc), false), __read(sliceAppointmentsByDays(appointment, excludedDays)), false)); }, []) : appointments[0];
    var expandedAppointments = slicedAppointments
        .reduce(function (acc, appointment) { return __spreadArray(__spreadArray([], __read(acc), false), __read(expandGroupedAppointment(appointment, grouping, resources)), false); }, []);
...
var getRowFromGroups = function (width, groupRow, cellStyle, groups, rowIndex) {
    ...
    var _loop_1 = function (i) {
        row = __spreadArray(__spreadArray([], __read(row), false), __read(groupRow.reduce(function (acc, group, index) { return __spreadArray(__spreadArray([], __read(acc), false), [
            {
                group: group,
                colSpan: colSpan,
...
var getLabelsForSingleGroup = function (groups, cellsData, groupIndex, groupHeight) {
    ...
    return cellsData.slice(currentGroupIndex, nextGroupIndex - 1).reduce(function (acc, days) { return ((__spreadArray(__spreadArray([], __read(acc), false), [
        {
...
var getLabelsForAllGroups = function (cellsData, groups, groupOrientation) {
    ...
    return getGroupsLastRow(groups).reduce(function (acc, group, groupIndex) { return __spreadArray(__spreadArray([], __read(acc), false), [
...
var getRecurrenceOptions = function (rule) { return !rule ? null : RRule.parseString(rule); };
...
var configureExDate = function (exDate, date) {
    ...
    return exDate
        ? "".concat(exDate, ",").concat(currentExDate)
        : currentExDate;
};
...
var getAppointmentResources = function (appointment, resources, plainResources) {
    ...
    return resources.reduce(function (acc, resource) {
        ...
        if (resource.allowMultiple) {
            return __spreadArray(__spreadArray([], __read(acc), false), __read(appointmentResourceId
                .reduce(function (prevResources, itemId) { return addResourceToAppointmentResources(plainResources, prevResources, resource, itemId); }, [])), false);
        }
...
var addResourceToAppointmentResources = function (plainResources, appointmentResources, resource, resourceId) {
    ...
    return currentResource ? __spreadArray(__spreadArray([], __read(appointmentResources), false), [
        currentResource,
    ], false) : appointmentResources;
};

var convertResourcesToPlain = function (validResources) {
    return validResources.reduce(function (acc, resource) { return __spreadArray(__spreadArray([], __read(acc), false), __read(resource.instances.map(function (item) { return item; })), false); }, []);
};
